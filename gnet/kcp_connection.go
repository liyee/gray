...
func newKcpServerConn(server giface.IServer, conn *kcp.UDPSession, connID uint64) giface.IConnection {
...
func newKcpClientConn(client giface.IClient, conn *kcp.UDPSession) giface.IConnection {
	c := &KcpConnection{
		conn:        conn,
...
func (c *KcpConnection) StartWriter() {
	glog.Ins().DebugF("Writer Goroutine is running")
	defer glog.Ins().DebugF("%s [conn Writer exit!]", c.RemoteAddr().String())
...
func (c *KcpConnection) StartReader() {
	glog.Ins().DebugF("[Reader Goroutine is running]")
	defer glog.Ins().DebugF("%s [conn Reader exit!]", c.RemoteAddr().String())
	defer c.Stop()
	defer func() {
...
func (c *KcpConnection) finalizer() {
	...
	glog.Ins().DebugF("Conn Stop()...ConnID = %d", c.connID)
}
...
func (c *KcpConnection) SetHeartBeat(checker giface.IHeartbeatChecker) {
	c.hc = checker
}
...
func (c *KcpConnection) GetMsgHandler() giface.IMsgHandler {
	return c.msgHandler
}
