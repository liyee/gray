...
func newWebsocketConn(server giface.IServer, conn *websocket.Conn, connID uint64) giface.IConnection {
...
func newWsClientConn(client giface.IClient, conn *websocket.Conn) giface.IConnection {
	c := &WsConnection{
		conn:        conn,
...
func (c *WsConnection) StartReader() {
	...
	for {
		...
		default:
			...
			n := len(buffer)
			if err != nil {
				glog.Ins().ErrorF("read msg head [read datalen=%d], error = %s", n, err.Error())
				return
			}
...
func (c *WsConnection) SetHeartBeat(checker giface.IHeartbeatChecker) {
	c.hc = checker
}
...
func (c *WsConnection) GetMsgHandler() giface.IMsgHandler {
	return c.msgHandler
}
